...
image=r'C:\Users\91760\Downloads\image3.jpg'
image = cv2.imread(r'C:\Users\91760\Downloads\image4.jpg')

# convert the image to grayscale
gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

# Using CannyImage , get the edges of the image
edgesCanny = cv2.Canny(gray_image, 100, 200)
...
contours, new = cv2.findContours(
    edgesCanny.copy(), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
...
for contour in contours:
    ...
    if len(approx) == 4:
        # Check if number plate image is RECTANGLE
        contour_numberplate = approx
        x_cord, y_cord, width, height = cv2.boundingRect(contour)
        numberplate = gray_image[y_cord:y_cord+height, x_cord: x_cord+width]
        if (numberplate.all):
            break
        else:
            continue
...
cv2.imshow("Number Plate Detection by Varun", image)
cv2.waitKey(0)